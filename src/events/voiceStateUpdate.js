...
module.exports = async (oldState, newState) => {
	...
	const penal = await penals.findOne({ guildID: newState.guild.id, userID: newState.member.id, type: 'VOICE-MUTE', active: true });
	const penal2 = await penals.findOne({ guildID: newState.guild.id, userID: newState.member.id, type: 'VOICE-MUTE', removed: false, temp: true, finishDate: { $lte: Date.now() } });
	...
	if(voiceLog) {
		...
		const vLog = client.channels.cache.get(voiceLog);
		...
		if(oldState.channel.id && oldState.serverMute && !newState.serverMute) {
			...
			const audit = await newState.guild.fetchAuditLogs({ type: 'MEMBER_UPDATE' });
