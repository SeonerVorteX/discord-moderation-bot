...
module.exports = async (guild, user) => {
	...
	const Embed = embed(user.username, user.avatarURL({ dynamic: true }), false);
	const audit = await guild.fetchAuditLogs({ type: 'MEMBER_BAN_ADD' });
	const executor = audit.entries.first().executor;
	const reason = await client.fetchBan(guild, user.id).then(bannedUser => bannedUser.reason);

	if(executor.bot && (executor.id == client.user.id || OtherBots.includes(executor.id))) return;

	const point = await client.addPenalPoint(guild.id, user.id, penalPoint);
	const penal = await client.newPenal(guild.id, user.id, 'BAN', true, executor.id, !reason ? 'Belirtilmedi!' : reason);
