...
module.exports = async (member) => {
	...
	// /Forbidden Tag
	const data = await forbiddenTag.findOne({ guildID: member.guild.id });
	...
	if(data && data.forbiddenTags.length && data.forbiddenTags.some(tag => member.user.username.includes(tag))) {
		...
		await client.wait(2000).then(() => {
			...
			const tag = data.forbiddenTags.find(tag => member.user.username.includes(tag));
	...
	// /Server Security
	const penal1 = await Penals.findOne({ guildID: member.guild.id, userID: member.id, type: 'JAIL', active: true });
	const penal2 = await Penals.findOne({ guildID: member.guild.id, userID: member.id, type: 'TEMP-JAIL', active: true });
	const penal3 = await Penals.findOne({ guildID: member.guild.id, userID: member.id, type: 'CHAT-MUTE', active: true });
	const penal4 = await Penals.findOne({ guildID: member.guild.id, userID: member.id, type: 'VOICE-MUTE', active: true });

	if(penal1 || penal2) {

		await client.wait(3000).then(() => member.roles.set(jailRoles));

		const channel = member.guild.channels.cache.get(securityLog);
