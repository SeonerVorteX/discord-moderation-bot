...
module.exports = async (message) => {
	...
	const Commands = await commands.findOne({ guildID: message.guild.id });
	...
	const Embed = embed('Mesaj Silindi', message.guild.iconURL({ dynamic: true }), false);
	const data = await snipe.findOneAndUpdate({ guildID: message.guild.id, channelID: message.channel.id }, { $set: { messageContent: message.content, userID: message.author.id, image: message.attachments.first() ? message.attachments.first().proxyURL : null, createdDate: message.createdTimestamp, deletedDate: Date.now() } }, { upsert: true });
	...
	if(entry && entry.action == 'MESSAGE_DELETE' && entry.extra.channel.id == message.channel.id && entry.target.id == message.author.id && !OtherBots.includes(entry.executor.id) && entry.executor.id !== client.user.id) {
		...
		if(messageLog && channel && channel.type == 'text') {
			channel.send(
				Embed
					.addFields(
