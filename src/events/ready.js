...
module.exports = async () => {
	...
	// Reload
	const data = await reload.findOne({ type: 'moderation' });
	...
	setInterval(async () => {
		...
		const Alarms = await alarms.find({ guildID: guild.id, finished: false });
		if(Alarms.length) {
			Alarms.filter(Alarm => Alarm.finishDate < Date.now()).forEach(async Alarm => {

				const alarmChannel = await guild.channels.cache.get(Alarm.channelID);
	...
	// Checking Penals
	const Embed = embed(false, false, false);
	setInterval(async () => {

		const datas = await Penals.find({ guildID: guild.id, active: true });
		...
		if(guild && datas.length) {
			...
			datas.filter(data => data.finishDate && data.finishDate < Date.now()).forEach(async (data, index) => {
				...
				if(data.type == 'TEMP-JAIL') {
					...
					data.active = false;
					await data.save();
					const user = await client.fetchUser(data.userID).then(user => user);
					const member = guild.members.cache.get(user.id);
					const staff = await client.fetchUser(data.staffID).then(user => user);
					const channel = client.channels.cache.get(jail.log);
					...
					if(jail.log && channel) {
						channel.send(Embed.setColor('#00FF00').setAuthor(member.user.username, member.user.avatarURL({ dynamic: true })).setDescription(`
...
**Jaillenme Sebebi :** \`${!data.reason ? 'Belirtilmedi!' : data.reason}\`
				...
				if(data.type == 'CHAT-MUTE') {
					...
					const member = guild.members.cache.get(user.id);
					const staff = await client.fetchUser(data.staffID).then(user => user);
					const channel = client.channels.cache.get(chatMute.log);
...
**Susturulma Sebebi :** \`${!data.reason ? 'Belirtilmedi!' : data.reason}\`
				...
				if(data.type == 'VOICE-MUTE') {
					...
					const member = guild.members.cache.get(user.id);
					const staff = await client.fetchUser(data.staffID).then(user => user);
					const channel = client.channels.cache.get(voiceMute.log);
