...
module.exports = {
	...
	execute(client, message, args, Embed) {
		...
		const onlineMembers = message.guild.members.cache.filter(member => member.presence.status !== 'offline').size.toString().replace(/ /g, ' ');
		const tagMembers = message.guild.members.cache.filter(member => guildTags.some(tag => member.user.username.includes(tag)) || member.user.discriminator == guildDiscriminator).size.toString();
		const voiceMembers = message.guild.members.cache.filter(member => member.voice.channel && !member.user.bot).size.toString();
		const botMembers = message.guild.members.cache.filter(member => member.user.bot && member.voice.channel).size.toString();
		const boostSize = message.guild.premiumSubscriptionCount;
