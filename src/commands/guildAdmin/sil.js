...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		if(messageLog) {
			...
			const mLog = message.guild.channels.cache.get(messageLog);

			if(mLog && mLog.type == 'text') {
				mLog.send(Embed.setDescription('').setAuthor('Toplu Mesaj Silindi', message.guild.iconURL({ dynamic: true })).setThumbnail(message.author.avatarURL({ dynamic: true })).addFields(
