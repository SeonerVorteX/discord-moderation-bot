...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(!data) return message.channel.true(message, Embed.setDescription(`Bu kanalda silinmiş herhangi bir mesaj bulunmuyor!`), { react: true });
...
\`Mesajın Silindiği Tarih :\` ${await client.duration(Date.now() - data.deletedDate)} önce
        `).setImage(data.image ? data.image : ``), { timeout: 8000, react: true });
