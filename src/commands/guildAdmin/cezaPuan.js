...
module.exports = {
	...
	async execute(client, message, args) {
		...
		if(!args[0]) {
			...
			const data = await penalPoints.findOne({ guildID: message.guild.id, userID: message.author.id });
			...
			const data = await penalPoints.findOneAndUpdate({ guildID: message.guild.id, userID: user.id }, { $inc: { penalPoint: point } }, { upsert: true });
			...
			const data = await penalPoints.findOne({ guildID: message.guild.id, userID: user.id });
