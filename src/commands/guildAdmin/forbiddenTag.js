...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const data = await forbiddenTag.findOne({ guildID: message.guild.id });
		...
		if(['ekle', 'add'].some(arg => args[0].toLowerCase() == arg)) {
			...
			const forbiddenMembers = message.guild.members.cache.filter(member => member.user.username.includes(args[1]) && !forbidRoles.some(role => member.roles.cache.has(role)));
			...
			const forbiddenMembers = message.guild.members.cache.filter(member => member.user.username.includes(args[1]));
			const onlineMembers = message.guild.members.cache.filter(member => member.user.username.includes(args[1]) && member.user.presence.status !== 'offline');
...
${forbiddenMembers.size < 10 && forbiddenMembers.size ? `\`>\` **Taga sahip kişilerin listesi :**\n${forbiddenMembers.map(member => member.toString()).join(' , ')}` : !forbiddenMembers.size ? '' : '`>` **Taga sahip kişilerin listesini görmek için sayfayı çevirin**'}
			...
			message.channel.send(Embed.setDescription(description)).then(async msg => {
				...
				let currentPage = 0;
				const pageArray = forbiddenMembers.array();
				const pages = pageArray.chunk(10);
			...
			const totalMembers = message.guild.members.cache.filter((member) => new RegExp(data.forbiddenTags.join('|'), 'g').test(member.user.username));
			...
			await new Promise(async (resolve) => {
				...
				data.forbiddenTags.forEach(async (tag, index) => {
					...
					const forbiddenMembers = message.guild.members.cache.filter(member => member.user.username.includes(tag));
