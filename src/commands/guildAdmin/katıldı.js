...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		if(['ver', 'dağıt'].some(arg => args[0].toLowerCase() == arg)) {
			...
			const members = message.guild.members.cache.filter(member => member.roles.cache.has(meetRole) && (!member.voice.channel || member.voice.channel.id !== message.member.voice.channel.id));
			...
			if(mark) message.react(mark);
			const members = message.guild.members.cache.filter(member => member.roles.cache.has(meetRole));
