...
module.exports = {
	...
	execute(client, message, args, Embed) {
		...
		if(!message.member.voice.channel && args[0]) {
			...
			const totalMembers = channel.members.filter(member => !Owners.includes(member.user.id) && !member.roles.cache.has(botYt) && !member.hasPermission(8) && !member.voice.serverMute).size;
