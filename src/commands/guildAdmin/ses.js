...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		if(!args[0]) {
			...
			const voiceData = await voiceJoinedAt.findOne({ guildID: message.guild.id, userID: message.author.id });
...
**Veri Bilgilerin :** ${!voiceData ? '`Veri Bulunamadı!`' : `Toplam \`${await client.duration(Date.now() - voiceData.date)}dir\` sestesin!`}
				...
				const voiceData = await voiceJoinedAt.findOne({ guildID: message.guild.id, userID: user.id });
...
**Veri Bilgileri :** ${!voiceData ? '`Veri Bulunamadı!`' : `Toplam \`${await client.duration(Date.now() - voiceData.date)}dir\` seste!`}
			...
			const voiceDatas = await voiceJoinedAt.find({ guildID: message.guild.id }).sort({ date: -1 });

			if(channel.members.size >= 15) {

				let currentPage = 1;
				const pageArray = channel.members.array();
