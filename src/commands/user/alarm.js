...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const datas = await alarms.find({ guildID: message.guild.id, userID: message.author.id, finished: false });
		...
		const Alarm = await client.newAlarm(message.guild.id, message.author.id, message.channel.id, reason ? reason : 'Belirtilmedi!', Date.now(), Date.now() + ms(duration.duration));
