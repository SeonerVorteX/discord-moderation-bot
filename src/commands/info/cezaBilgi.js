...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const penal = await penals.findOne({ id: id, guildID: message.guild.id });
		...
		const user = await client.fetchUser(penal.userID);
		const staff = await client.fetchUser(penal.staffID);
