...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const clientStat = user.user.presence.clientStatus ? Object.keys(user.user.presence.clientStatus).map(activity => clientStatus[activity]).join(' , ') : 'Bilinmiyor!';
		const userStat = userStatus[user.user.presence.status];
		const customStatus = user.user.presence.activities.filter(activity => activity.type == 'CUSTOM_STATUS').length && user.user.presence.activities.find(activity => activity.type == 'CUSTOM_STATUS').state ? user.user.presence.activities.find(activity => activity.type == 'CUSTOM_STATUS').state : '';
		const emoji = user.user.presence.activities.filter(activity => activity.type == 'CUSTOM_STATUS').length && user.user.presence.activities.find(activity => activity.type == 'CUSTOM_STATUS').emoji && !user.user.presence.activities.find(activity => activity.type == 'CUSTOM_STATUS').emoji.id ? `${user.user.presence.activities.find(activity => activity.type == 'CUSTOM_STATUS').emoji.toString()} ` : '';
