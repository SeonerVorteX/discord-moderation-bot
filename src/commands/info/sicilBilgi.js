	...
	async execute(client, message, args, Embed) {
		...
		if(user) {
			...
			message.channel.send(Embed.setDescription(firstPage).setFooter(`${Footer} • Sayfa : 1`)).then(async msg => {
				...
				let currentPage = 1;
				const pages = Penals.sort((a, b) => a.date - b.date).chunk(10);
				const reactions = ['◀', '❌', '▶'];
