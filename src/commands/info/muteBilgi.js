...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const user = message.mentions.members.first() || message.mentions.users.first() || message.guild.members.cache.get(args[0]) || client.users.cache.get(args[0]) || await client.fetchUser(args[0]).then(user => user);
		...
		const penal = await Penals.findOne({ guildID: message.guild.id, userID: user.id, type: 'CHAT-MUTE', active: true }) || await Penals.findOne({ guildID: message.guild.id, userID: user.id, type: 'VOICE-MUTE', active: true });
