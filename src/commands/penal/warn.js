...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const penal = await client.newPenal(message.guild.id, user.id, 'WARN', false, message.author.id, !reason ? 'Belirtilmedi!' : reason);
		const datas = await Penals.find({ guildID: message.guild.id, userID: user.id, type: 'WARN' });
		...
		if(log) {
			client.channels.cache.get(log).send(Embed.setDescription(`
