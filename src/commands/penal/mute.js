...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        let staffDatas = await Penals.find({ guildID: message.guild.id, type: 'CHAT-MUTE', staff: message.author.id });
        ...
        message.channel.true(message, Embed.setDescription(`${chatMuted} \`${user.user.tag}\` isimli kullanıcı, ${message.author.toString()} tarafından, ${!reason ? '' : `\`${reason}\` sebebiyle`}, **${duration.durationMsg}** boyunca susturuldu! \`(Ceza ID : #${penal.id})\``), { react: true });
...
**Susturulma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(dmMessages) user.send(`${chatMuted} \`${message.guild.name}\` sunucusunda **${message.author.tag}** tarafından **metin kanallarında** ${!reason ? '' : `\`${reason}\` sebebiyle`}, **${duration.durationMsg}** boyunca susturuldunuz! \`(Ceza ID : #${penal.id})\``).catch(() => {});
        if(logs.pointLog) client.channels.cache.get(logs.pointLog).send(`${chatMuted} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **ChatMute** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
...
**Susturulma Sebebi :** \`${!penal.reason ? 'Belirtilmedi!' : penal.reason}\`
            ...
            if(dmMessages) user.send(`${unCMuted} \`${message.guild.name}\` sunucusunda, **${message.author.tag}** tarafından, ${!reason ? '' : `\`${reason}\` sebebiyle`} **metin kanallarında** aldığınız susturulma cezasının süresi bitti! \`(Ceza ID : #${penal.id})\``).catch(() => {});
