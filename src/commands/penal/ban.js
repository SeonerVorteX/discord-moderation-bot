...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const staffDatas = await Penals.find({ guildID: message.guild.id, type: 'BAN', staffID: message.author.id });
		...
		const penal = await client.newPenal(message.guild.id, user.id, 'BAN', true, message.author.id, !reason ? 'Belirtilmedi!' : reason);
		...
		if(log) {
			client.channels.cache.get(log).send(Embed.setColor('#FF0000').setFooter('').setImage(banGifs.random()).setDescription(`
