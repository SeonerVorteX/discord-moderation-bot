...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const staffDatas = await Penals.find({ guildID: message.guild.id, type: 'JAIL', staffID: message.author.id });
...
**Jaillenme Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
		...
		setTimeout(async () => {
			...
			if(log) {
				message.guild.channels.cache.get(log).send(Embed.setColor('#00FF00').setDescription(`
