...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(await client.fetchBan(message.guild.id, args[0])) return message.channel.error(message, `Belirttiğin üye sunucudan daha önce yasaklanmış!`, { timeout: 8000, react: true })
        ...
        message.channel.true(message, Embed.setDescription(`${banned} \`${user.user.tag}\` isimli kullanıcı, ${message.author.toString()} tarafından, ${!reason ? '' : `\`${reason}\` sebebiyle`} sunucudan **kalıcı olarak** yasaklandı! \`(Ceza ID : #${penal.id})\``), { react: true });
...
**Yasaklanma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(dmMessages) user.send(`${banned} \`${message.guild.name}\` sunucusunda, **${message.author.tag}** tarafından, ${!reason ? '' : `\`${reason}\` sebebiyle`} **kalıcı olarak** yasaklandınız! \`(Ceza ID : #${penal.id})\``).catch(() => {});
        if(pointLog) client.channels.cache.get(pointLog).send(`${banned} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **ForceBan** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
