...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const staffDatas = await Penals.find({ guildID: message.guild.id, type: 'VOICE-MUTE', staffID: message.author.id });
...
**Susturulma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
		...
		setTimeout(async () => {
			...
			if(log) {
				client.channels.cache.get(log).send(Embed.setColor('#00FF00').setDescription(`
