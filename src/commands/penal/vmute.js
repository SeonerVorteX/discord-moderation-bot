...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        let staffDatas = await Penals.find({ guildID: message.guild.id, type: 'VOICE-MUTE', staff: message.author.id });
        ...
        message.channel.true(message, Embed.setDescription(`${muted} \`${user.user.tag}\` isimli kullanıcı ${message.author.toString()} tarafından **ses kanallarında** ${!reason ? '' : `\`${reason}\` sebebiyle`}, **${duration.durationMsg}** boyunca susturuldu! \`(Ceza ID : #${penal.id})\``), { react: true });
...
**Susturulma Sebebi :** \`${!reason ? 'Belirtilmedi!' : reason}\`
        ...
        if(dmMessages) user.send(`${muted} \`${message.guild.name}\` sunucusunda **${message.author.tag}** tarafından **ses kanallarında** ${!reason ? '' : `\`${reason}\` sebebiyle`}, **${duration.durationMsg}** boyunca susturuldunuz! \`(Ceza ID : #${penal.id})\``).catch(() => {});
        if(logs.pointLog) client.channels.cache.get(logs.pointLog).send(`${muted} ${user.toString()}, aldığınız \`#${penal.id}\` ID'li **VoiceMute** cezası ile toplam **${point.penalPoint}** ceza puanına ulaştınız!`);
...
**Susturulma Sebebi :** \`${!penal.reason ? 'Belirtilmedi!' : penal.reason}\`
            ...
            if(dmMessages) user.send(`${unMuted} \`${message.guild.name}\` sunucusunda, **${message.author.tag}** tarafından, ${!reason ? '' : `\`${reason}\` sebebiyle`} **ses kanallarında** aldığınız susturulma cezasının süresi bitti! \`(Ceza ID : #${penal.id})\``).catch(() => {});
