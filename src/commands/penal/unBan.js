...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const bannedUser = await client.fetchBan(message.guild, args[0]);
		...
		const forceban = await Penals.findOne({ guildID: message.guild.id, userID: bannedUser.user.id, type: 'FORCE-BAN', active: true });
		const forceBan = await forceBans.findOne({ guildID: message.guild.id, userID: bannedUser.user.id });
		...
		const penal = await Penals.findOne({ guildID: message.guild.id, userID: bannedUser.user.id, type: 'BAN', active: true });
		...
		if(log) {
			client.channels.cache.get(log).send(Embed.setColor('#00FF00').setFooter('').setImage(unbanGifs.random()).setDescription(`
