...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const staffDatas = await Penals.find({ guildID: message.guild.id, type: 'JAIL', ID: message.author.id });
		const staffDatas2 = await Penals.find({ guildID: message.guild.id, type: 'TEMP-JAIL', staffID: message.author.id });
		const dataSize = staffDatas.filter(staffData => staffData.date && (Date.now() - staffData.date) < 3600 * 1000);
		const dataSize2 = staffDatas2.filter(staffData => staffData.date && (Date.now() - staffData.date) < 3600 * 1000);
		...
		const point = await client.addPenalPoint(message.guild.id, user.id, penalPoint);
		const penal = await client.newPenal(message.guild.id, user.id, 'JAIL', true, message.author.id, !reason ? 'Belirtilmedi!' : reason);
		...
		if(log) {
			client.channels.cache.get(log).send(Embed.setColor('#FF0000').setDescription(`
