...
module.exports = {
  ...
  async execute(client, message, args, Embed){
    ...
    let code = args.join(' ');
    if(message.channel.type == 'text') message.delete({reason: "eval"});
    function clean(text) {
    ...
    try { 
      ...
      if(evaled.match(new RegExp(`${client.token}`, 'g'))) evaled.replace("token", "Secret Information").replace(client.token, "Secret Information");
