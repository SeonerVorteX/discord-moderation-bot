...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const channel2 = message.mentions.channels.array()[1] || message.guild.channels.cache.get(args[1]);
		...
		if(channel) {
			...
			const totalMembers = channel.members.filter(member => !Owners.includes(member.user.id) && ((member.hasPermission(8) && member.user.bot && !OtherBots.includes(member.user.id)) && member.user.id !== client.user.id || !member.hasPermission(8))).size;
			let index = 0;
