...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		const tagMembers = message.guild.members.cache.filter(member => member.roles.cache.has(role.id) && (guildTags.filter(tag => tag !== '').some(tag => member.user.username.includes(tag)) || member.user.discriminator == guildDiscriminator)).size;
		const onlineMembers = message.guild.members.cache.filter(member => member.roles.cache.has(role.id) && member.user.presence.status !== 'offline').size;
		const voiceMembers = message.guild.members.cache.filter(member => member.roles.cache.has(role.id) && member.voice.channelID).size;
		const description = `
...
${role.members.size < 10 && role.members.size ? `\`>\` **Role sahip kişilerin listesi :**\n${role.members.map(member => member.toString()).join(' , ')}` : !role.members.size ? '' : '`>` **Role sahip kişilerin listesini görmek için sayfayı çevirin**'}
		...
		message.channel.send(Embed.setDescription(description)).then(async msg => {
			...
			let currentPage = 0;
			const pageArray = role.members.array();
