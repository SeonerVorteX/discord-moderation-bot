...
module.exports = {
	...
	async execute(client, message, args, Embed) {
		...
		if(['ekle', 'oluştur', 'add'].some(arg => args[0].toLocaleLowerCase() == arg)) {
			...
			const data = await staffs.findOne({ guildID: message.guild.id, staffName: name });
			...
			const staff = await new staffs({ guildID: message.guild.id, authorID: message.author.id, staffName: name }).save();
			...
			if(user) {
				...
				const members = role.members.filter(member => data.staffRoles.some(role => !member.roles.cache.has(role)));
				...
				const members = channel.members.filter(member => data.staffRoles.some(role => !member.roles.cache.has(role)));
				const size = members.size;
				let index = 0;
...
**Yetkiye sahip üye sayısı :** \`${data.staffs && data.staffs.length ? message.guild.members.cache.filter(member => data.staffRoles.every(role => member.roles.cache.has(role))).size : 0}\`
			...
			const firstPage = datas.sort((a, b) => a.date - b.date).splice(0, 5).map((data, index) => `
...
**Yetkiye sahip üye sayısı :** \`${data.staffs && data.staffs.length ? message.guild.members.cache.filter(member => data.staffRoles.every(role => member.roles.cache.has(role))).size : 0}\`
			...
			message.channel.send(Embed.setDescription(firstPage)).then(async msg => {
				...
				let currentPage = 1;
				const pages = datas.sort((a, b) => a.date - b.date).chunk(5);
				const reactions = ['◀', '❌', '▶'];
